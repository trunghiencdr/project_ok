...
public class TramDao {
    ...
    public void suaTramTrongDB(String tenTramCu, Tram tram) {
        try {
            LopKetNoi.update("update tram set tenTram = ?, diaChi = ?  where tenTram = ?",  tram.getTenTram(), tram.getDiaChi(), tenTramCu);
        } catch (Exception e) {
        }
    }
